    ...
    while open_set:
        ...
        for next in get_neighbors(maze, current):
            ...
            if next not in cost_so_far or new_cost < cost_so_far[next]:
                ...
                log_message(
                    f"A\*: Node ({next[0]},{next[1]}) - Cost: {new_cost}, Priority: {priority}"
                )
...
class Mouse(pygame.sprite.Sprite):
    ...
    def update(self, maze, end):
        ...
        global current_step, explored_cells  # Access the global variables directly
        if not self.path:
...
def reset_simulation():
    global maze, start, end, mouse, all_sprites, simulation_running, explored_cells, current_step
...
button_step_forward = Button(
    MAZE_WIDTH * CELL_SIZE + 130, 120, 40, 40, ">", GRAY, (100, 100, 100), step_forward
)
